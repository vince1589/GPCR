...
def optimize(atmsel, sched):
    #conjugate gradient
    for step in sched:
        step.optimize(atmsel, max_iterations=500, min_atom_shift=0.001)
    #md
    refine(atmsel)
    cg = conjugate_gradients()
    cg.optimize(atmsel, max_iterations=500, min_atom_shift=0.001)
...
mdl1.write(file='dummy.tmp')
mdl1.read(file='dummy.tmp')
...
si = selection(mdl1.chains[chain].residues[respos])
s = selection.only_sidechain(si)
...
s.energy()
#score = mdl1.assess_ga341()
#give a proper name
mdl1.write(file='temp.pdb')